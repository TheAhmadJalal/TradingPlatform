...
async function loadClient() {
  const res = await fetch(`${API_URL}/crm-api/clients/${acc}`);
  ...
  if (saveButton) {
    saveButton.addEventListener("click", async () => {
      ...
      const res = await fetch(`${API_URL}/crm-api/clients/${acc}/comment`, {
        method: "PATCH",
  ...
  async function loadTradeCounts() {
    const res = await fetch(`${API_URL}/crm-api/clients/${acc}/trade-counts`);
    const data = await res.json();
  ...
  async function loadTransactionCount() {
    const res = await fetch(`${API_URL}/crm-api/clients/${acc}/transaction-count`);
    const data = await res.json();
  ...
  saveTradeBtn.onclick = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/crm-api/trades`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
document.getElementById("transactionForm")?.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (data.type !== "bonus" && data.type !== "credit") {
      const res = await fetch(`${API_URL}/crm-api/transactions`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    if ((data.type === "bonus" || data.type === "credit") && data.status === "successful") {
      await fetch(`${API_URL}/crm-api/clients/update-financials`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId: data.userId,
  ...
  try {
    const res = await fetch(`${API_URL}/crm-api/clients/${acc}/documents`);
    const data = await res.json();
    list.innerHTML = "";
...
document.getElementById("viewTransactionsBtn")?.addEventListener("click", async () => {
  ...
  try {
    const res = await fetch(`${API_URL}/crm-api/transactions/user/${clientId}`);
    const data = await res.json();
...
async function loadTradesForUser(userId) {
  ...
  try {
    const res = await fetch(`${API_URL}/crm-api/trades/user/${clientId}`);
